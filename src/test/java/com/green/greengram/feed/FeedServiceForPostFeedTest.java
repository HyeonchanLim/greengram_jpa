...
class FeedServiceForPostFeedTest extends FeedServiceParentTest {
    ...
    @Test
    @DisplayName("Insert시 영향받은 행이 0 일 때 , 예외 발생")
    void postFeedInsRows0ThrowException(){
        ...
        // givenParam 을 가져오는 FeedPostReq 에 @EqualsAndHashCode 필수 !
        // 그래야 값 비교가 가능함
    ...
    void test2() throws Exception {
        ...
        given(feedMapper.insFeed(givenParam)).will(invocation -> {
            ...
            // 아래에 actualParam 주소값이 넘어온다
            invocationParam.setFeedId(FEED_ID_10);
            // actualParam 에 feedId = 10 이 담기게 된다
            return 1;
            // insFeed 메소드의 리턴값
        });
        ...
        // void 메소드인 경우 given(임무 부여) 방법
        // 주의사항 : when() 안에서 메소드 호출을 작성하면 안된다.
        doAnswer(invoctaion -> {
        ...
        assertAll(
                ...
                // verify 는 메소드가 호출되었는지 확인함
                // myFileUtils 에 makeFolders 메소드가 호출됐는지 체크 ->
                // 메소드 안에 파라미터값 ("/feed/10") 이 들어왔는지 단언
    ...
    @Test
    @DisplayName("test2 와 같은 맥락 , 파일을 2개로 테스트 함. test2_1 보다는 test2가 낫다")
